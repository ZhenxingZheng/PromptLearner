...
def validate(epoch, val_loader, device, model_image, model_text, fusion_model, config, num_text_aug):
    model_image.eval()
    model_text.eval()
    fusion_model.eval()
    num = 0
    ...
    with torch.no_grad():
        list_ids = torch.arange(config.data.num_classes).to(device)
        text_ids = torch.arange(num_text_aug).to(device)
        list_ids = torch.cat([list_ids.unsqueeze(1)] * num_text_aug, dim=1)
        list_ids = list_ids.flatten()
        text_ids = torch.cat([text_ids] * config.data.num_classes, dim=0).flatten()
        text_features, text_features_sequence = model_text(text_ids, list_ids)
        ...
        for iii, (image, class_id) in enumerate(tqdm(val_loader)):
            ...
            image_features = model_image(image_input).view(b, t, -1)
...
def main():
    ...
    best_prec1 = 0.0
    prec1 = validate(start_epoch, val_loader, device, model, fusion_model, config, num_text_aug)

...
class Action_DATASETS(data.Dataset):
    def __init__(self, list_file, labels_file,
                 ...
                 random_shift=False, test_mode=False, index_bias=0, test_mode_augment=False):
        ...
        self.test_mode = test_mode
        self.test_mode_augment = test_mode_augment

        if self.index_bias is None:
            if self.image_tmpl == "{}.jpg":
                self.index_bias = 0
            else:
                self.index_bias = 0
        self._parse_list()
    ...
    def get_indices(self, record):
        ...
        choices = [np.random.choice(average_duration, 1, replace=False) + i * average_duration for i in range(self.num_segments)]
        choices = np.concatenate((choices))
        offsets = np.sort(choices) + self.index_bias
        return offsets

    def __getitem__(self, index):
        record = self.video_list[index]
        if not (self.test_mode and self.test_mode_augment):
            segment_indices = self._sample_indices(record) if self.random_shift else self._get_val_indices(record)
            return self.get(record, segment_indices)
        else:
            process_data = []
            for count in range(10):
                segment_indices = self.get_indices(record)
                video_data, temp = self.get(record, segment_indices)
                process_data.append(video_data)

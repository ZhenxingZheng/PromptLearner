...
from modules.graph_att import GAttNet as GAT
from modules.language_model import QuestionSelfAttention
...
def q_expand_v_cat(q, v, mask=True):
    ...
    q_expand = q.expand(*repeat_vals).clone() # [batch, num_rois, 1024]
    if mask:
        v_sum = v.sum(-1)
...
class ImplicitRelationEncoder(nn.Module):
    ...
    def forward(self, v, position_embedding, q):
        ...
        for i in range(self.num_steps):
            v_cat_q = q_expand_v_cat(q, imp_vs[i], mask=True)
            imp_v_rel = self.implicit_relation.forward(v_cat_q,
                                                       imp_adj_mat,
                                                       position_embedding)
            if self.residual_connection:
                imp_vs.append(imp_vs[i] + imp_v_rel)
            else:
                imp_vs[i+1] = imp_v_rel
        return imp_vs[i+1]
